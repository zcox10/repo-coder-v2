...
def main(params):
    ...
    # dset = h5py.File(params['output_h5'],"r+")
    ...
    for i, img in enumerate(imgs):
        ...
        #   continue
        # load the image
        I = io.imread(os.path.join(params["images_root"], img["file_path"]))
        # print(i," read")
        # print(np.max(I),np.min(I))
        try:
            Ir = imresize(I, (256, 256))
            # Ir = np.resize(np.array(I), (256,256))
            # print("resized")
        except:
            print(
                "failed resizing image %s - see http://git.io/vBIE0"
                % (img["file_path"],)
            )
            raise
