...
# json_file_path = os.path.join('data/','cocotalk.json')
...
class Network(nn.Module):
    ...
    def __init__(
        self, C, layers, criterion, decoder, steps=4, multiplier=4, stem_multiplier=3
    ):
        ...
        self._decoder = decoder.cuda()
        self.lin = nn.Linear(C_prev, self._decoder.hidden_size).cuda()
        # self.classifier = nn.Linear(self._decoder.hidden_size, num_classes)
    ...
    def enc_params(self):
        return (
            ...
            + list(self.cells.parameters())
            + list(self.global_pooling.parameters())
            + list(self.lin.parameters())
        )

    def dec_params(self):
        return list(self._decoder.parameters()) + list(self.lin.parameters())
    ...
    def forward(self, input, alphas, captions, lengths):
        ...
        out = self.global_pooling(s1)
        out = self.lin(out.view(out.size(0), -1))
        # print("model_search out: ", out.shape)
        logits, _ = self._decoder(out, alphas, captions, lengths)

        return logits

    def loss(self, input, alphas, captions, lengths):
        # print("before stuff: ", input.shape, captions.shape)
        logits = self(input, alphas, captions, lengths)
        # targets = pack_padded_sequence(captions, lengths, batch_first=True)[0]
        # print("after stuff: ",logits.shape, captions.shape)
        # print("loss comparison: ",self._criterion(logits, captions.view(-1)), F.cross_entropy(logits, captions.view(-1).long()))
        return self.criterion(logits, captions.view(-1))
